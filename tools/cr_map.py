"""CommonRoad: visualise (PNG), crop-box, and crop-interactive (click or drag box)."""
...
# Interactive crop needs a GUI backend before pyplot loads.
...
def crop_scenario_to_lanelets(scenario, keep_ids, name_suffix: str) -> None:
...
def cmd_visualise(args):
    ...
    out = args.output
    if out is None:
        stem = p.stem[:-3] if p.stem.endswith(".cr") else p.stem
        out = p.parent / (stem + "_map.png")
    else:
        out = out.resolve()
...
def parse_bbox(args):
    ...
    if args.top_right and args.bottom_right:
        ...
        if abs(trx - brx) > 1e-3:
            raise SystemExit("top-right and bottom-right need same x; use --box for a general rectangle")
        if args.left_x is None:
            raise SystemExit("need --left-x with --top-right and --bottom-right")
        xmin, xmax = sorted((float(args.left_x), float(trx)))
        ymin, ymax = sorted((float(bry), float(try_)))
        return xmin, xmax, ymin, ymax
    raise SystemExit("need --box X1 Y1 X2 Y2 OR --top-right/--bottom-left OR --top-right/--bottom-right/--left-x")


def cmd_crop_box(args):
    ...
    print("bbox", bbox, "kept", len(keep), "->", out)
    return 0


def build_parser():
    r = argparse.ArgumentParser(description="CommonRoad visualise + crop-box")
    s = r.add_subparsers(dest="command", required=True)
    v = s.add_parser("visualise", help="PNG map")
    ...
    v.set_defaults(_fn=cmd_visualise)
    c = s.add_parser("crop-box", help="axis-aligned crop in map metres")
    c.add_argument("input_xml", type=Path)
    c.add_argument("output_xml", type=Path)
    c.add_argument("--box", type=float, nargs=4, metavar=("X1", "Y1", "X2", "Y2"), default=None)
    c.add_argument("--top-right", type=float, nargs=2, metavar=("X", "Y"), default=None)
    c.add_argument("--bottom-left", type=float, nargs=2, metavar=("X", "Y"), default=None)
    c.add_argument("--bottom-right", type=float, nargs=2, metavar=("X", "Y"), default=None)
    c.add_argument("--left-x", type=float, default=None)
    c.add_argument("--name-suffix", default="CropBox")
    c.set_defaults(_fn=cmd_crop_box)
    return r


def main(argv=None):
    argv = list(sys.argv[1:] if argv is None else argv)
    if argv and argv[0] == "crop-interactive":
        import subprocess
        script = Path(__file__).resolve().parent / "crop_interactive.py"
        if not script.is_file():
            raise SystemExit(f"Missing {script}")
        subprocess.check_call([sys.executable, str(script)] + argv[1:])
        return 0
